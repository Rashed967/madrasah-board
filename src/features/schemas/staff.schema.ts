...
const baseStaffSchema = z.object({
  ...
  nidNumber: z
    .string()
    .min(10, 'সঠিক এনআইডি নম্বর দিন')
    .max(17, 'সঠিক এনআইডি নম্বর দিন'),
  code: z.string().optional()
})

// Muhtamim specific schema
export const createMuhtamimSchema = baseStaffSchema.extend({
  highestEducationalQualification: z
    .string()
    .min(1, 'শিক্ষাগত যোগ্যতা দিতে হবে')
})

// Mutawalli specific schema
export const createMutawalliSchema = baseStaffSchema.extend({
  designation: z.enum(
    chairmanMutawalliDesignation.map((item) => item.value) as [
      string,
      ...string[]
    ],
    {
...
export const updateEducationalSecretarySchema =
  createEducationalSecretarySchema.partial()
