...
export const getAllKitabs = async (): Promise<{
  success: boolean
  data: Kitab[]
  message: string
}> => {
...
export const getAllMarhalas = async (
  fields?: string,
  populate?: boolean,
  page: number = 1,
  limit: number = 50,
  marhalaType?: string
): Promise<ApiResponse<IMarhala[]>> => {
  try {
    ...
    if (marhalaType !== undefined)
      queryParams.append('marhalaType', marhalaType.toString())

    const queryString = queryParams.toString()
    const url = `/marhalas${queryString ? `?${queryString}` : ''}`

    const response = await get<IMarhala[]>(url)
    return response
  } catch (error) {
    return {
      success: false,
...
export const createMarhala = async (
  marhalaData: CreateMarhalaData
): Promise<ApiResponse<IMarhala[]>> => {
  try {
...
export const deleteMarhala = async (
  id: string
): Promise<ApiResponse<IMarhala>> => {
  try {
...
export const updateMarhala = async (
  id: string,
  marhalaData: Partial<CreateMarhalaData>
): Promise<ApiResponse<IMarhala>> => {
  try {
