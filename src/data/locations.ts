...
export const getDistricts = (division: string): string[] => {
  ...
  Object.keys(subDistrictsData).forEach((district) => {
    if (
      ...
      division === 'সিলেট' &&
      ['সিলেট', 'মৌলভীবাজার', 'হবিগঞ্জ', 'সুনামগঞ্জ'].includes(district)
    ) {
      districts.push(district)
    } else if (
      division === 'রংপুর' &&
      ...
      division === 'ময়মনসিংহ' &&
      ['ময়মনসিংহ', 'জামালপুর', 'শেরপুর', 'নেত্রকোনা'].includes(district)
    ) {
      districts.push(district)
    }
  })
...
export const districts = divisions.reduce(
  (acc, division) => {
...
export const policeStations = Object.keys(policeStationsData).reduce(
  (acc, district) => {
    const districtData = policeStationsData[district]
