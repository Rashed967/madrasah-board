...
export default function EditMarhalaDialog({
  marhala,
  isOpen,
  onClose,
  onSuccess
  ...
  useEffect(() => {
    if (marhala) {
      setFormData({
        ...
        level: marhala.level || 0,
        listOfKitabs: marhala.listOfKitabs
          .map((k) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const originalKitabs = marhala?.listOfKitabs
        .map((k) => (typeof k === 'string' ? k : k._id?.toString()))
        .filter(Boolean)
      const currentKitabs = formData.listOfKitabs
      ...
      const marhalaId =
        typeof marhala?._id === 'string' ? marhala?._id : marhala?._id?.toString()
      ...
      if (response.success) {
        if (response.data.level !== formData.level) {
          toast.success(`মারহালা আপডেট করা হয়েছে। লেভেল ${formData.level} এর পরিবর্তে ${response.data.level} সেট করা হয়েছে।`, { duration: 5000 })
        } else {
    ...
    listOfKitabs: [] as (string | { _id: string })[]
        ...
        const errorMessage =
          response.message || 'মারহালা আপডেট করতে সমস্যা হয়েছে'
    ...
    <Dialog className="md:w-6/12" isOpen={isOpen} onClose={onClose} title="মারহালা সম্পাদনা">
      ...
      >
        <FormInput
          ...
          onChange={(value) => setFormData(prev => ({ ...prev, bengaliName: value }))}
          placeholder="মারহালার নাম লিখুন"
          required
        />

        <FormInput
          label="মারহালার নাম (আরবি)"
          value={formData.arabicName}
          onChange={(value) => setFormData(prev => ({ ...prev, arabicName: value }))}
        ...
        <FormSelect
          ...
          onChange={(value) => setFormData(prev => ({ ...prev, marhalaType: value }))}
          placeholder="মারহালার ধরণ নির্বাচন করুন"
          required
          options={[
        ...
        <FormSelect
          ...
          onChange={(value) => setFormData(prev => ({ ...prev, marhalaCategory: value }))}
          placeholder="মারহালার ক্যাটাগরি নির্বাচন করুন"
          required
          options={[
            { value: 'darsiyat', label: 'দারসিয়াত' },
            { value: 'hifz', label: 'হিফজ' }
          ]}
        />

        <FormInput
          label="মারহালার লেভেল"
          value={formData.level}
          onChange={(value) => setFormData(prev => ({ ...prev, level: Number(value) }))}
          placeholder="মারহালার লেভেল"
          required
        />
        ...
        <div>
          ...
          <div className="space-y-4">
            <div className="flex gap-2">
              <Select value={selectedKitab} onValueChange={setSelectedKitab}>
                ...
                <SelectContent>
                  {availableKitabs.map((kitab) => (
                    <SelectItem
                      key={kitab._id?.toString()}
                      value={kitab._id?.toString() || ''}
                    >
            ...
            <div className="mt-2 space-y-2">
              {formData.listOfKitabs.map((kitabId) => {
                const kitab = availableKitabs.find(
                  (k) =>
                    k._id?.toString() ===
                    (typeof kitabId === 'object' ? kitabId._id : kitabId)
                )
                const actualKitabId =
                  typeof kitabId === 'object' ? kitabId._id : kitabId
                return (
