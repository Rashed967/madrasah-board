...
const courierTypes = ['কুরিয়ার', 'ডাক', 'উভয়']
...
const LocationForm: React.FC<Props> = ({ formData, handleChange, errors }) => {
  ...
  useEffect(() => {
    if (formData.address.district) {
      ...
      // Reset dependent fields
      if (formData.address.subDistrict_policeStation) {
        handleChange('address.subDistrict_policeStation', '')
        handleChange('address.postOffice', '')
      }
    } else {
      setSubDistricts([])
    }
  ...
  useEffect(() => {
    if (formData.address.district && formData.address.subDistrict_policeStation) {
      const policeStations = getPoliceStations(
        formData.address.district,
        formData.address.subDistrict_policeStation
      )
      setPoliceStationList(policeStations)
      // Reset dependent field
      if (formData.address.postOffice) {
        handleChange('address.postOffice', '')
      }
    } else {
      setPoliceStationList([])
    }
  }, [formData.address.district, formData.address.subDistrict_policeStation])

  return (
    <div className=" p-6">
      <h2 className="text-lg font-semibold mb-4 text-gray-800">ঠিকানা</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <select
            ...
            className={`mt-1 block w-full rounded-md border ${
              errors?.['address.division']
                ? 'border-red-500'
                : 'border-gray-300'
            } px-3 py-2 text-sm md:text-base text-gray-700 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 h-[38px]  overflow-y-auto`}
          >
          ...
          <select
            ...
            className={`mt-1 block w-full rounded-md border ${
              errors?.['address.district']
                ? 'border-red-500'
                : 'border-gray-300'
            } px-3 py-2 text-sm md:text-base text-gray-700 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 h-[38px] [&+div]:max-h-[200px] [&+div]:overflow-y-auto ${
        ...
        <div>
          ...
          <select
            value={formData.address.subDistrict_policeStation}
            onChange={(e) =>
              handleChange('address.subDistrict_policeStation', e.target.value)
            }
            disabled={!formData.address.district}
            className={`mt-1 block w-full rounded-md border ${
              errors?.['address.subDistrict_policeStation']
                ? 'border-red-500'
                : 'border-gray-300'
            } px-3 py-2 text-sm md:text-base text-gray-700 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 h-[38px] [&+div]:max-h-[200px] [&+div]:overflow-y-auto ${
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              handleChange('address.holdingNumber', e.target.value)
            }
            className={`mt-1 block w-full rounded-md border ${
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              handleChange('address.courierAddress', e.target.value)
            }
            className={`mt-1 block w-full rounded-md border ${
